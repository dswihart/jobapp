...
export class RemotiveSecuritySource extends BaseJobSource {
  ...
  async fetchJobs(skills: string[], limit: number = 50): Promise<JobPosting[]> {
    try {
      this.log('Fetching jobs from Remotive Security RSS...')

      const response = await fetch('https://remotive.com/remote-jobs/feed/software-dev')
      ...
      this.log(`Received ${jobs.length} jobs from Remotive Security`)
