...
export class EuRemoteJobsSource extends BaseJobSource {
  config: JobSourceConfig = {
    name: 'EuRemoteJobs Security',
    enabled: true,
    type: 'rss' as const,
  ...
  async fetchJobs(skills: string[], limit: number = 50): Promise<JobPosting[]> {
    try {
      this.log('Fetching jobs from EuRemoteJobs Security RSS...')

      const response = await fetch('https://euremotejobs.com/feed/?s=security')
      ...
      this.log(`Received ${jobs.length} jobs from EuRemoteJobs Security`)
