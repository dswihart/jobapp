...
export class BarcelonaSecurity2Source extends BaseJobSource {
  config: JobSourceConfig = {
    name: 'Barcelona Cyber Jobs',
    enabled: true,
    type: 'rss' as const,
  ...
  async fetchJobs(skills: string[], limit: number = 50): Promise<JobPosting[]> {
    try {
      ...
      const response = await fetch('https://rss.app/feeds/v1.1/8v16VEBGjIYkFkpH.json')
