/**
 * technoemploye Job Source
 * Auto-generated on 2025-10-04T20:05:59.628Z
 */

import { BaseJobSource, JobPosting, JobSourceConfig } from '../job-source-interface'

interface technoemployeSourceJob {
...
export class technoemployeSource extends BaseJobSource {
  ...
  private readonly API_URL = 'https://rss.app/feeds/v1.1/KyyZhabhz2dSAVXa.json'
  // If your API needs authentication, add:
  // private readonly API_KEY = process.env.TECHNOEMPLOYE_API_KEY
  ...
  async fetchJobs(skills: string[], limit: number = 50): Promise<JobPosting[]> {
    try {
      ...
      const skillFiltered = this.filterBySkills(
        jobs,
        skills,
        (job: technoemployeSourceJob) => `${job.title} ${job.description}`
      )
      ...
      const recentJobs = this.filterByRecency(
        skillFiltered,
        7,
        (job: technoemployeSourceJob) => new Date(job.posted_date || new Date())
      )

      // Map to standard JobPosting format
      const standardJobs: JobPosting[] = recentJobs
        .slice(0, 20)
        .map((job: technoemployeSourceJob) => ({
          title: job.title,
          company: job.company,
