...
export class Foorilla1Source extends BaseJobSource {
  config: JobSourceConfig = {
    name: 'Foorilla Security Jobs',
    enabled: true,
    type: 'rss' as const,
  ...
  async fetchJobs(skills: string[], limit: number = 50): Promise<JobPosting[]> {
    try {
      this.log('Fetching jobs from Foorilla Security RSS...')

      const response = await fetch('https://foorilla.com/account/feed/0SqKXfDIRw65VD0/rss/')
      ...
      this.log(`Received ${jobs.length} jobs from Foorilla Security RSS`)
  ...
  private parseXMLFeed(xmlText: string, limit: number): JobPosting[] {
    ...
    while ((match = itemRegex.exec(xmlText)) !== null && count < limit) {
      ...
      if (title && link) {
        ...
        const location = locationMatch ? locationMatch[1].trim() : 'Spain'
