...
export class RemoteOKSecuritySource extends BaseJobSource {
  ...
  async fetchJobs(skills: string[], limit: number = 50): Promise<JobPosting[]> {
    try {
      this.log('Fetching jobs from RemoteOK Security RSS...')

      const response = await fetch('https://remoteok.com/remote-security+engineer-jobs.rss')
      ...
      this.log(`Received ${jobs.length} jobs from RemoteOK Security`)
