...
export class CyberSecRemoteSource extends BaseJobSource {
  config: JobSourceConfig = {
    name: 'CyberSecurity Remote Jobs',
    enabled: true,
    type: 'rss' as const,
  ...
  async fetchJobs(skills: string[], limit: number = 50): Promise<JobPosting[]> {
    try {
      this.log('Fetching jobs from CyberSecurity Remote RSS...')

      const response = await fetch('https://rss.app/feeds/X52InKYY4d72H6H7.xml')
      ...
      this.log(`Received ${jobs.length} jobs from CyberSecurity Remote RSS`)
  ...
  private parseXMLFeed(xmlText: string, limit: number): JobPosting[] {
    ...
    while ((match = itemRegex.exec(xmlText)) !== null && count < limit) {
      ...
      if (title && link && !link.includes('/logon/')) {
        jobs.push({
          title,
