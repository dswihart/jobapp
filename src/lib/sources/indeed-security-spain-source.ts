...
export class IndeedSecuritySpainSource extends BaseJobSource {
  config: JobSourceConfig = {
    name: 'Indeed Security Engineer Spain',
    enabled: true,
    type: 'rss' as const,
  ...
  async fetchJobs(skills: string[], limit: number = 50): Promise<JobPosting[]> {
    try {
      this.log('Fetching jobs from Indeed Security Engineer Spain RSS...')

      const response = await fetch('https://rss.indeed.com/rss?q="security+engineer"&l=Spain&remotejob=1')
      ...
      this.log(`Received ${jobs.length} jobs from Indeed Security Engineer Spain`)
