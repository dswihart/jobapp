...
export class SecurityJobs4Source extends BaseJobSource {
  config: JobSourceConfig = {
    name: 'Security Jobs Feed 4',
    enabled: true,
    type: 'rss' as const,
  ...
  async fetchJobs(skills: string[], limit: number = 50): Promise<JobPosting[]> {
    try {
      this.log('Fetching jobs from Security RSS 4...')

      const response = await fetch('https://rss.app/feeds/v1.1/7SQuJ0MlE9azN6Cz.json')
